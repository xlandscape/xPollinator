...
### Option 2: add xPollinator to any Landscape Model
...
2. Create an xPollinator folder in *\core\components* if it does not already exist.
3. Copy the xPollinator component from [GitHub](https://github.com/xlandscape/xPollinatorDemo/tree/main) into the xPollinator subfolder.
4. The file *mc.xml* contains information about the components that are used in the created [xLandscape](xLandscape/xLandscape-intro.md) model. Make use of the xPollinator component by adding the following lines:

``` xml
<xPollinator module="components" class="xPollinator">
    <xPollinatorFilePath scales="global">
        $(_PROJECT_DIR_)\CropProtection\$(CropProtectionScenario).xml
    </xPollinatorFilePath>
    <ParametrizationNamespace scales="global">
        urn:xPollinatorLandscapeScenarioParametrization
...
To start xPollinator using the sample scenario, **drag *template.xrun* onto *__start_\_.bat***. This will start an xPollinator run using the demo scenario. Output of the model run can be found in the newly created *\run\Rummen-demo-scenario\mcs\\[mc run ID]\store\arr.dat*.
